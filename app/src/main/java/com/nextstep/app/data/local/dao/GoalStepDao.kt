...
@Dao
interface GoalStepDao : SyncDao<GoalStepEntity> {
    ...
    override suspend fun getById(id: String): GoalStepEntity?
    ...
    override suspend fun upsert(item: GoalStepEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun upsertAll(items: List<GoalStepEntity>)

    @Query("SELECT * FROM goal_steps WHERE familyId = :familyId AND dirty = 1")
    override suspend fun getDirty(familyId: String): List<GoalStepEntity>
