...
object GoalMapper : EntityMapper<GoalEntity> {
    ...
    override fun toMap(entity: GoalEntity): Map<String, Any?> = with(entity) {
        mapOf(
            ...
            "status" to status.name, "createdByRole" to createdByRole, "createdAt" to createdAt, "updatedAt" to updatedAt, "deleted" to deleted,
        )
    }
    ...
    override fun fromMap(id: String, data: Map<String, Any?>): GoalEntity = GoalEntity(
        ...
        description = data.str("description"), status = GoalStatus.from(data.strOrNull("status")), createdByRole = data.str("createdByRole"),
