...
object GoalStepMapper : EntityMapper<GoalStepEntity> {
    ...
    override fun fromMap(id: String, data: Map<String, Any?>): GoalStepEntity = GoalStepEntity(
        ...
        title = data.str("title"), detail = data.str("detail"), status = MilestoneStatus.from(data.strOrNull("status")), taskId = data.strOrNull("taskId"), dueDate = data.longOrNull("dueDate"),
