...
object MemberMapper : EntityMapper<MemberEntity> {
    ...
    override fun toMap(entity: MemberEntity): Map<String, Any?> = with(entity) {
        mapOf(
            ...
            "mentorEnabled" to mentorEnabled, "gradeYear" to gradeYear, "birthDate" to birthDate, "joinedAt" to joinedAt, "updatedAt" to updatedAt, "deleted" to deleted,
        )
    }
    ...
    override fun fromMap(id: String, data: Map<String, Any?>): MemberEntity = MemberEntity(
        ...
        subjectIds = data.str("subjectIds"), mentorEnabled = data.bool("mentorEnabled"), gradeYear = data.int("gradeYear"), birthDate = data.longOrNull("birthDate"), joinedAt = data.long("joinedAt"),
