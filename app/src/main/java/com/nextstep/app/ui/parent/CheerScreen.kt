                ...
                AppCard {
                    Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        ...
                        state.cheerSuggestions.forEach { s ->
                            AssistChip(onClick = { viewModel.send(s) }, label = { Text(s) }, leadingIcon = { Icon(Icons.Default.Favorite, contentDescription = null, tint = MaterialTheme.colorScheme.error) })
                        }
                    }
                }
            }
            ...
            if (state.notes.isEmpty()) item { AppCard { EmptyState("첫 응원 메시지를 보내 보세요") } }
                ...
                AppCard {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        if (n.authorName == state.myName && n.authorRole == Role.PARENT.name) TextButton(onClick = { viewModel.delete(n.id) }) { Text("삭제") }
                    }
                }
            }
