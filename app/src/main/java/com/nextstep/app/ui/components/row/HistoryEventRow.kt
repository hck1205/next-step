...
fun HistoryEventRow(e: HistoryEvent) {
    ...
    Row(verticalAlignment = Alignment.Top, horizontalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Column(Modifier.weight(1f)) {
            ...
            val who = Role.from(e.byRole)?.let { PlanHistory.assignerLabel(it) }
            val sub = listOfNotNull(
                e.kind.label, who?.let { if (e.kind != HistoryKind.TASK_DONE) "$it 만든 목표" else if (e.byRole == Role.STUDENT.name) "스스로 정한 일" else "$it 준 일" },
