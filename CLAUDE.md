...
- 해마다 할 일은 `domain/year` 에 분류(`YearArea`)·때(`YearTerm`)·누가(`YearDoer`)로 적는다: 학업·생활 `YearPlans`, 건강 `HealthPlans`, 부모 `ParentPlans`, 멘토 `MentorPlans`(합친 결과는 `YearPlans.forYear`). 공식 일정·금액은 각 파일 KDoc 의 출처와 함께 고치고, 학생 "올해" 탭(`ui/yearplan`)이 그 분류를 탭으로 보여 준다. 학년·생년월일·화면 단계는 1년을 가는 값이라 가족 탭의 요약 한 줄 + "고치기" 창에서만 바꾼다(칩을 화면에 늘어놓지 않는다).
