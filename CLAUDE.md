...
- 게임 요소(레벨·배지·연속·이번 주 도전)는 `domain/gamify/Gamify` 가 기존 기록에서 계산한다(저장하는 점수 없음, 해낸 일·꾸준함에만 경험치, 감점·비교 없음). 켜고 끄기는 학생의 `MemberEntity.gamify`(학부모만, `caps.canToggleGamification`). 보상은 `RewardEntity`(목표 달성·레벨에만, 할 일마다 걸지 않음)로 학부모·멘토가 약속하고 준다(`caps.canGiveRewards`), 상태 계산은 `domain/reward/Rewards` 한 곳.
