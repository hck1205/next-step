...
- 학생 화면의 학년별 차이는 domain 에서만 정한다: 단계(보이는 카드·말투·탭)는 `domain/growth/StudentUiLevel.kt`, 해마다 달라지는 공부 종류·양·글씨·카드 순서는 `domain/growth/YearProfiles.kt`, 둘을 합친 결과는 `StudentScreen.of(student)`. 화면에서 학년으로 분기하지 말고 `level.shows(StudentHomeSection.X)`·`level.words` 를 쓴다. 기록 탭에서 섹션이 보이는 학년은 `domain/hub/ConcernSection.minLevel` 한 곳에서 정한다.
