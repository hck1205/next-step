...
- 해마다 할 일은 `domain/year` 에 분류(`YearArea`)·때(`YearTerm`)·누가(`YearDoer`)로 적는다: 학업·생활 `YearPlans`, 건강 `HealthPlans`, 부모 `ParentPlans`, 멘토 `MentorPlans`(합친 결과는 `YearPlans.forYear`). 학교부터 교과 줄은 단원 단위 + 도달 기준(`YearTask.bar`, "이만큼이면 충분")을 적는다. 기본 위의 "앞서 가기"(`YearTier.AHEAD`)는 `AheadPlans` 한 곳에 도착점·까닭(`why`)과 함께 적고 진행률에 넣지 않는다(깊게·넓게 먼저, 다음 학년 내용은 겨울방학에 개념만, 학령 전은 놀이·대화만). 공식 일정·금액은 각 파일 KDoc 의 출처와 함께 고치고, 학생 "올해" 탭(`ui/yearplan`)이 그 분류를 탭으로 보여 준다. 학년·생년월일·화면 단계는 1년을 가는 값이라 가족 탭의 요약 한 줄 + "고치기" 창에서만 바꾼다(칩을 화면에 늘어놓지 않는다).
- 사람이 만드는 목표는 목표 트리(`domain/goaltree`)로 적는다: 목표 = GoalEntity(trackId `tree`, `leadsTo` 로 더 큰 목표에 이어짐, 순환 금지), 세부 할 일 = TaskEntity(`goalId`, 끝낸 시각 `doneAt` 이 기록). 달성률은 `GoalNode.rate`(할 일 + 작은 목표), 먼저 볼 것(달성 대기·밀림·멈춤·비어 있음)은 `GoalNode.attention`, 누가 줬는지는 `createdByRole` → 문구는 `Assigner` 한 곳. 기한이 있어도 미션(`MissionPlanner.isMission`)이 아니다. 시스템 추천 할 일은 `domain/taskboard/TaskSuggester` 한 곳에서 모으고 과목별 줄(`TaskBoard`)로 보여 준다. 할 일 체크 권한은 `caps.canCheckTask(stage)`.
- 자기주도는 `domain/selfdirection` 의 사다리(`SelfDirectionStage`: 따라 해요 → 골라요 → 같이 계획해요 → 먼저 계획해요 → 스스로 해요 → 내가 주인)로만 정한다. 단계마다 계획·실행·점검·돌아보기를 누가(어른이·같이·스스로) 맡는지가 있고, 기본값은 화면 단계, 학부모가 준비 신호(`SelfDirection.suggest`)를 보고 한 칸씩 바꾼다(`MemberEntity.selfDirection`). 화면은 `caps.canDo(step, stage)`·`WeekAccess` 로만 버튼을 켜고 끈다. 주간 계획·돌아보기는 `WeekPlanEntity`(주마다 한 행).
- 게임 요소(레벨·배지·연속·이번 주 도전)는 `domain/gamify/Gamify` 가 기존 기록에서 계산한다(저장하는 점수 없음, 해낸 일·꾸준함에만 경험치, 감점·비교 없음). 나이별 모양(스티커판 → 레벨·배지 → 성장 기록: 도전·배지·연속의 쉬는 날·보상 거는 곳)은 `StudentUiLevel.game`(`GameStyle`) 한 곳에서 정하고 화면은 `profile.style` 만 본다. 켜고 끄기는 학생의 `MemberEntity.gamify`(`caps.canToggleGamification(style)`: 학부모, 성장 기록이면 학생도). 보상은 `RewardEntity`(목표 달성·레벨에만, 할 일마다 걸지 않음)로 학부모·멘토가 약속하고 준다(`caps.canGiveRewards`), 상태 계산은 `domain/reward/Rewards` 한 곳.
- 학업과 다른 "오래 키우는 힘"(영어·독서·악기·운동…)은 `domain/project` 의 교육 프로젝트로 적는다: 목표 한 문장 → 단계(하루 루틴 · 통과 기준 · 준비물) → 루틴 기록. 계획은 `ProjectCatalog`(id·단계 key 는 바꾸지 않는다), 계산은 `ProjectPlanner`. 저장은 목표 저장소(trackId `project:`, 단계 periodKey `project:<key>`, targetDate 없음)와 루틴 기록 `ProjectLogEntity` 라서 시험·목표 목록·여정 학기 단계·공부 시간에 섞이지 않는다.
