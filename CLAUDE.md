...
- 사람이 만드는 목표는 목표 트리(`domain/goaltree`)로 적는다: 목표 = GoalEntity(trackId `tree`, `leadsTo` 로 더 큰 목표에 이어짐, 순환 금지), 세부 할 일 = TaskEntity(`goalId`, 끝낸 시각 `doneAt` 이 기록). 달성률은 `GoalNode.rate`(할 일 + 작은 목표), 누가 줬는지는 `createdByRole`. 기한이 있어도 미션(`MissionPlanner.isMission`)이 아니다. 시스템 추천 할 일은 `domain/taskboard/TaskSuggester` 한 곳에서 모으고 과목별 줄(`TaskBoard`)로 보여 준다. 할 일 체크 권한은 `caps.canCheckTask(stage)`.
